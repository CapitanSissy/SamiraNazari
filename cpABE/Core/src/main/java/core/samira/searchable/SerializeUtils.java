...
import core.samira.searchable.SKComponent;
...
//          Class<?> clazz = type.getComponentType();
          ...
          if (clazz == SKComponent.class || clazz == SecretKey.class) {
          ...
          } else if (clazz == PublicKey.class) {
            PKComponent[] array = (PKComponent[]) field.get(obj);
            int len = array.length;
            dos.writeByte(SimpleSerializable.ArrayMark);
            dos.writeInt(len);
            for (int i = 0; i < len; i++) {
              PKComponent comp = array[i];
              _serialize(comp, dos);
            }
          } else if (clazz == MasterKey.class) {
            MKComponent[] array = (MKComponent[]) field.get(obj);
            int len = array.length;
            dos.writeByte(SimpleSerializable.ArrayMark);
            dos.writeInt(len);
            for (int i = 0; i < len; i++) {
              MKComponent comp = array[i];
              _serialize(comp, dos);
            }
          } else if (clazz == Pedersen.class) {
            Pedersen array = (Pedersen) field.get(obj);
            int len = fields.length;
            dos.writeByte(SimpleSerializable.ElementMark);
            dos.writeInt(len);
            for (int i = 0; i < len; i++) {
//              Pedersen comp = array[i];
//              _serialize(comp, dos);
            }
          }
//          else if () {
